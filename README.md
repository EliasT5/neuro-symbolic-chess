...
- LLM (default: `model-o-4o-mini`, configurable via `NSC_LLM_MODEL`).
...
- Works offline (without `MODEL_O_API_KEY`) via a small heuristic — useful for tests.
...
A real `FastMCP` server exposing the same `chess.*` surface used internally by the engine. Mirrors Jerome's broker tools so external MCP clients (Model-C Code, the MCP Inspector, anyone else) can use this repo as a chess legality oracle.
...
export MODEL_O_API_KEY=sk-...        # optional; without it, S1 uses an offline heuristic
...
For the full session journal — what was built, what almost didn't work, and why the architecture is the shape it is — see [`docs/2026-05-03-field-report.md`](docs/2026-05-03-field-report.md).
